...
$req = mysqli_query($link, 'select * from mailbox');

while($dnn = mysqli_fetch_array($req))
{
	echo $dnn['id'].",";
	echo htmlentities($dnn['subject'], ENT_QUOTES, 'UTF-8').",";
...
while($dnn = mysqli_fetch_array($req))
{
	...
	echo htmlentities($dnn['mskey'], ENT_QUOTES, 'UTF-8').",";
	echo htmlentities($dnn['tag'], ENT_QUOTES, 'UTF-8')."<BR>";
}
?>
	</body>
